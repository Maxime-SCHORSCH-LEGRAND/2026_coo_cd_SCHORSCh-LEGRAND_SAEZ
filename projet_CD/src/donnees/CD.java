...
public class CD {
	...
	private String getNomCD(){
		return this.nomCD;
	}
	private String getArtiste(){
	...
	public int compareCD(CD autre) {
		return this.getNomCD().compareToIgnoreCase(autre.getNomCD());
	}

	public int compareArtiste(CD autre) {
		return this.getArtiste().compareToIgnoreCase(autre.getArtiste());
	}
	public void trierSelection() {
		...
		while (!this.listeCds.isEmpty()) {
			...
			for (int i = 1; i < this.listeCds.size(); i++) {
				String titreCourant = this.listeCds.get(i).getTitreCD();
				String titreMin = this.listeCds.get(indiceMin).getTitreCD();
				if (titreCourant.compareToIgnoreCase(titreMin) < 0) {
					indiceMin = i;
				}
			}
			CD plusPetit = this.listeCds.get(indiceMin);
			listeTriee.add(plusPetit);
			this.listeCds.remove(indiceMin);
		}
		this.listeCds = listeTriee;
	}
}
